...
if ($con) {
    ...
    $get_date_query = mysqli_query($con, "select tour_check_available from tour_details");
    while($get_date_res = mysqli_fetch_array($get_date_query)){
        $get_dates = $get_date_res['tour_check_available'];
        $get_dates_explode = explode(',', $get_dates);

        $i = 0;
        foreach ($get_dates_explode as $key => $get_dates_avail) {
            if($get_dates_avail > date('Y-m-d')){ 
                if($i < 1){                   
                    $get_dates_available[] = $get_dates_avail;                   
                }
                $i++ ;
            }
        }
    }
    $get_greater_dates = $get_dates_available;
    sort($get_greater_dates);
    $greater_first_date = $get_greater_dates[0];

    //echo "select * from tour_details where find_in_set('".$greater_first_date."', tour_check_available)";

    $query = mysqli_query($con, "select * from tour_details where find_in_set('".$greater_first_date."', tour_check_available) limit 0,1");
    if (mysqli_num_rows($query) > 0) {
        ...
        $json = array("status" => 1, "msg" => "Upcoming Tour Details", "Upcoming List" => $tour_list);
