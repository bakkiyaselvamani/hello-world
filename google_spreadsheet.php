...
class google_spreadsheet
{
	...
	function addRow($row)
	{
		...
		if ($this->client instanceof Zend_Gdata_Spreadsheets)
		{
			...
			$ss_id = '1elZnFPfSQm7ulTTzFuUuax2rD3zdN2SGEnW-x1MP9lU';//$this->getSpreadsheetId($this->spreadsheet);
			if (!$ss_id) throw new Exception('Unable to find spreadsheet by name: "' . $this->spreadsheet . '", confirm the name of the spreadsheet');

			$ws_id = '2PACX-1vSDJMWaDUbwD6wCLrdJF8uFVVplmZdBOadfuMyfQGBkc1tf2LmanRpCqtqV32KbFeMqwepfzUMRJfE1';//$this->getWorksheetId($ss_id,$this->worksheet);
